...
export async function GET(req: Request) {
  ...
  while (true) {
    // Fetch next batch of eligible users
    ...
    for (const row of rows) {
      try {
        ...
        const ok = await sendWeeklyDigestEmail(user.email, name, {
          thisWeekLessons, totalLessons: completedCount, streak, srDue,
          trackLabel, nextLessonTitle, nextLessonUrl, unsubscribeUrl,
        })
        ...
        if (ok) {
          ...
          await supabase.from('email_log').insert({
            user_id: row.user_id, email: user.email,
            sequence: 'weekly-digest', step: 1, status: 'sent',
          })
        } else {
          skipped++
        }
