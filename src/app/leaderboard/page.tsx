  ...
  return (
    <tr>
      <td style={{ padding: '14px 20px' }}>
        <div style={{ width: 24, height: 18, background: '#F1F5F9', borderRadius: 3 }} />
      </td>
      <td style={{ padding: '14px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#F1F5F9' }} />
          <div style={{ width: 80 + (rank % 3) * 20, height: 14, background: '#F1F5F9', borderRadius: 3 }} />
        </div>
      </td>
      {[1, 2, 3, 4, 5].map((i) => (
        <td key={i} style={{ padding: '14px 20px' }}>
          <div style={{ width: 40, height: 14, background: '#F1F5F9', borderRadius: 3 }} />
        </td>
      ))}
    </tr>
  )
  ...
  useEffect(() => {
    fetch('/api/leaderboard')
      ...
      .then((data: LeaderboardEntry[]) => {
        setEntries(data)
        if (user) {
          const found = data.find((e) => e.userId === user.id)
                        ...
                        >
                          <td style={{ padding: '14px 20px', width: 56 }}>
                            {entry.rank <= 3 ? (
                              ...
                              <span style={{
                                fontSize: 14,
                                fontWeight: entry.rank <= 3 ? 800 : 500,
                                color: '#64748B',
                              }}>
                                {entry.rank}
                              </span>
                          ...
                          <td style={{ padding: '14px 20px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                              ...
                              <span style={{
                                ...
                                {entry.name}
                                {isMe && (
                                  <span style={{ fontSize: 12, color: '#2563EB', fontWeight: 600, marginLeft: 6 }}>
                                    (you)
                                  </span>
                                )}
                              </span>
