      ...
      <motion.div
        ...
        <div className="rounded-2xl overflow-hidden"
          ...
          <div className="px-8 pt-8 pb-6 text-center"
            style={{ background: 'linear-gradient(135deg, #EDE9FE, #F0FDF4)', borderBottom: '1px solid #E2E8F0' }}>
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4"
              style={{ background: '#FFFFFF', boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
              <Users size={26} style={{ color: '#7C3AED' }} />
            </div>
            <p className="text-xs font-bold tracking-widest uppercase mb-2" style={{ color: '#7C3AED' }}>Team invitation</p>
            <h1 className="text-2xl font-black mb-1" style={{ color: '#0F172A' }}>
              You've been invited to join
            </h1>
            <p className="text-xl font-black" style={{ color: '#7C3AED' }}>{teamName}</p>
          </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {assignedTracks.map(t => (
                    <span key={t}
                      ...
                      style={{
                        background: `${TRACK_COLORS[t] ?? '#7C3AED'}12`,
                        color: TRACK_COLORS[t] ?? '#7C3AED',
                        border: `1px solid ${TRACK_COLORS[t] ?? '#7C3AED'}30`,
                      }}>
                      <BookOpen size={10} />
                      {TRACK_LABELS[t] ?? t}
                    </span>
